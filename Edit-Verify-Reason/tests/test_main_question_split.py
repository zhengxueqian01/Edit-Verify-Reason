...
class MainQuestionSplitTests(unittest.TestCase):
    ...
    def test_resolve_questions_can_skip_explicit_decomposition(self) -> None:
        llm = _StubLLM(
            json.dumps(
                {
                    "operation_text": "Delete one series.",
        ...
        operation_text, qa_question, split_info, data_change = _resolve_questions(
            {
                ...
                "auto_split_question": False,
            },
            llm,
        )

        self.assertEqual(operation_text, "Delete one series and tell me which year has the maximum value")
        self.assertEqual(qa_question, "Delete one series and tell me which year has the maximum value")
        self.assertEqual(split_info["reason"], "disabled")
        self.assertEqual(data_change, {})
