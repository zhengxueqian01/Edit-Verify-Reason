...
def _plan_tool_calls(
    *,
    ...
    calls, filtered = _filter_explicit_answer_markup_tools(question=question, chart_type=chart_type, tool_calls=calls)
    if filtered:
...
def _filter_explicit_answer_markup_tools(
    *,
    question: str,
    chart_type: str,
    tool_calls: list[dict[str, Any]],
    ...
    answer_like_labels = _explicit_answer_label_patterns(question=question, chart_type=chart_type)
    ...
    for call in tool_calls:
        ...
        if tool not in {"add_point", "draw_line", "highlight_rect"}:
            kept.append(call)
            continue

        if normalized_chart_type == "area":
            rejected.append({"tool": tool, "args": args, "reason": "area_markup_tool_forbidden"})
            continue

        label = str(args.get("label") or "").strip()
        if label and _label_explicitly_marks_answer(label, answer_like_labels):
            rejected.append({"tool": tool, "args": args, "reason": "explicit_answer_markup"})
            continue
        kept.append(call)
...
def _explicit_answer_label_patterns(*, question: str, chart_type: str) -> list[re.Pattern[str]]:
    patterns = [
        re.compile(
            r"\b("
            r"answer|overall maximum|overall minimum|maximum|minimum|max|min|peak|highest|lowest|"
            r"top value|bottom value|year of maximum|year of minimum|year of peak|"
            r"point of maximum|point of minimum|intersection|crossing|cluster count"
            r")\b",
            flags=re.IGNORECASE,
        ),
        re.compile(r"^\d{4}$"),
    ...
    if str(chart_type or "").strip().lower() == "area":
        if "overall maximum" in lowered_question or "overall minimum" in lowered_question:
            patterns.append(re.compile(r"\byear\b", flags=re.IGNORECASE))
    return patterns


def _label_explicitly_marks_answer(label: str, patterns: list[re.Pattern[str]]) -> bool:
