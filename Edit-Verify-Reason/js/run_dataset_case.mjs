...
function findProjectRoot(startDir) {
  ...
  while (true) {
    if (
      fs.existsSync(path.join(current, "Edit-Verify-Reason", "chart_agent")) &&
      fs.existsSync(path.join(current, "Evaluation"))
    ) {
      return current;
    }
...
function parseArgs(argv) {
  ...
  if (!args.task || !args.case) {
    throw new Error("Usage: node Edit-Verify-Reason/js/run_dataset_case.mjs --task <task> --case <id> [options]");
  }
