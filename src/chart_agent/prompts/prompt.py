...
def build_visual_tool_planner_prompt(
    *,
    ...
    return (
        ...
        "  * scatter: default to isolate_all_color_topologies to add same-color background halos for all points; use isolate_color_topology only when a single target color is explicitly required.\n"
        "  * line: for intersection/counting questions, first use isolate_target_lines to fade unrelated lines, then use zoom_and_highlight_intersection to mark crossings.\n"
        "  * area: use highlight_top_boundary when the task benefits from tracing the upper envelope.\n"
    ...
    *,
    ...
    if chart_type == "scatter":
        if answer_stage == "tool_augmented":
            return ANSWER_TOOL_AUGMENTED_SCATTER_SYSTEM_PROMPT
        if _is_scatter_cluster_question(
            qa_question=qa_question,
            data_summary=data_summary,
            chart_type=chart_type,
        ):
            return ANSWER_UPDATED_SCATTER_SYSTEM_PROMPT
        return ANSWER_UPDATED_SCATTER_SYSTEM_PROMPT
    if chart_type == "line":
        return ANSWER_TOOL_AUGMENTED_LINE_SYSTEM_PROMPT if answer_stage == "tool_augmented" else ANSWER_UPDATED_LINE_SYSTEM_PROMPT
    if chart_type == "area":
        return ANSWER_TOOL_AUGMENTED_AREA_SYSTEM_PROMPT if answer_stage == "tool_augmented" else ANSWER_UPDATED_AREA_SYSTEM_PROMPT
    return ""
